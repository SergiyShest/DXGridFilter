...
function FilterElements(datagrid, fe) {
    ...
    this.CreateFilter = createFilter; //создание контролов
    this.SetOuterFilter = setOuterFilter; //поиск контролов по идентификатору и создание по ним массива

    this.AddDevexpressCheckBox = addDevexpressCheckBox;

    function addDevexpressCheckBox(checkBox, dataField) {
        var column = { dataField: dataField, dataType: "number" };
        var field = new DevexpressCheckBoxFilterField(column, checkBox, FilterFind);
    ...
    function createFilter(columns) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            var field = new FilterField(column, input, input2, checkBox, FilterFind);
    ...
    //Функция создания фильтра
    function setOuterFilter(columns) {
