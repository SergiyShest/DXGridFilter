const FilterHelper = require('../DXGridFilter/Content/filterHelper.js');
//import { FilterHelper }  from '../dist/FilterHelper.js';

test('test simple not', () => {
        var exp = "([U] = 10.0m And [i] = 20s)";
     var resFilter = FilterHelper.RemoveFromExpession(exp, 'id').trim();
    expect(resFilter).toBe('[U] = 10.0m And [i] = 20s');
    
  });

  test.each([

    ["([U] = 10.0m And [i] = 20s)", "([U] = 10.0m And [i] = 20s)"],
  ])('.FilterHelper.RemoveFromExpession(exp, "id").trim();', (expr, expected) => {
